...
NAME = $(or $(shell grep -h '^NAME=' openshift/parameters/* 2>/dev/null | uniq | awk -F= '{print $$2}'), CHARACTER_BUILDER)
NAMESPACE = $(or $(shell grep -h '^[^\#]*NAMESPACE=' openshift/parameters/* 2>/dev/null | uniq | awk -F= '{print $$2}'), CHARACTER_BUILDER)

OC_TEMPLATE_DIR = $(TOPDIR)/openshift
OC_PARAM_DIR = $(OC_TEMPLATE_DIR)/parameters
OC_TEMPLATES = $(wildcard $(OC_TEMPLATE_DIR))

CHARACTER_BUILDER_DOCKER_HASH = $(shell docker ps -q -f name=CHARACTER_BUILDER_server)
...
help:
	...
	@echo "  docker-up-CHARACTER_BUILDER                       run docker-compose up -d CHARACTER_BUILDER-server"
	...
	@echo "  oc-create-CHARACTER_BUILDER-api                    create the CHARACTER_BUILDER API pod"
	@echo "  oc-create-CHARACTER_BUILDER-auth-cache             create the Redis pod for auth caching"
	...
	@echo "  oc-create-route                       create routes for CHARACTER_BUILDER APIs"
	...
	@echo "  oc-delete-CHARACTER_BUILDER-api                    delete the CHARACTER_BUILDER API pod"
	@echo "  oc-delete-CHARACTER_BUILDER-auth-cache             delete the Redis pod for auth caching"
